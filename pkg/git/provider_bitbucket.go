...
import (
	"context"
	"fmt"
	"gitbucket.com/google/go-gitbucket/v32/gitbucket"
	"strings"
)

// GitbucketProvider represents a Gitbucket API wrapper
type GitbucketProvider struct {
	IsPrivate  bool
	IsPersonal bool
}

const (
	GitbucketTokenName       = "GITBUCKET_TOKEN"
	GitbucketDefaultHostname = "gitbucket.com"
)

func (p *GitbucketProvider) newClient(r *Repository) (*gitbucket.Client, error) {
	auth := gitbucket.BasicAuthTransport{
		Username: "git",
		Password: r.Token,
	}

	gh := gitbucket.NewClient(auth.Client())
	if r.Host != GitbucketDefaultHostname {
		baseURL := fmt.Sprintf("https://%s/api/v3/", r.Host)
		uploadURL := fmt.Sprintf("https://%s/api/uploads/", r.Host)
		if g, err := gitbucket.NewEnterpriseClient(baseURL, uploadURL, auth.Client()); err == nil {
			gh = g
		} else {
			return nil, err
		}
...
func (p *GitbucketProvider) CreateRepository(ctx context.Context, r *Repository) (bool, error) {
	...
	autoInit := true
	_, _, err = gh.Repositories.Create(ctx, org, &gitbucket.Repository{
		AutoInit: &autoInit,
		Name:     &r.Name,
		Private:  &p.IsPrivate,
	})
...
func (p *GitbucketProvider) AddTeam(ctx context.Context, r *Repository, name, permission string) (bool, error) {
	gh, err := p.newClient(r)
	if err != nil {
		return false, fmt.Errorf("client error: %w", err)
	}
	...
	if resp.StatusCode == 404 {
		_, err = gh.Teams.AddTeamRepoBySlug(ctx, r.Owner, name, r.Owner, r.Name, &gitbucket.TeamAddTeamRepoOptions{
...
func (p *GitbucketProvider) AddDeployKey(ctx context.Context, r *Repository, key, keyName string) (bool, error) {
	gh, err := p.newClient(r)
	if err != nil {
		return false, fmt.Errorf("client error: %w", err)
	}
	...
	if shouldCreateKey {
		isReadOnly := true
		_, _, err = gh.Repositories.CreateKey(ctx, r.Owner, r.Name, &gitbucket.Key{
			Title:    &keyName,
			Key:      &key,
			ReadOnly: &isReadOnly,
		})
